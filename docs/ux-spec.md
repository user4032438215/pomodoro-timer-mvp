...
- **終了時の挙動**  
  ...
  - アクションセンターで通知、音でも知らせる。ユーザーに次のセッションに移行するか、laterにするかの選択肢は与えておいたほうがよさそう。
- **操作性**  
  - スタート／一時停止ボタン ／ ポモドーロそのものを終了するボタン
  - 誤操作防止→最初の2択以降は停止再開のトグルボタン、ポモドーロそのものを終了するボタン、以外非表示にする仕様
  - タスクそのものを終了させるボタン→誤操作防止＋終了前にユーザーへやさしい後押しが目的のナッジ付きダイアログの実装予定。
- **モチベーション演出**  
  - ~~完了タスクごとに🔥アイコンを表示~~
  - 『現在〇ポモドーロ完了！』みたいなのを画面上のどこかしらに表示させる。
...
- **現在の検討ポイント**
  ...
  - サインインオプションとしてカレンダーに1日の作業時間と1か月のトータルの時間を記録する機能を追加することを検討
